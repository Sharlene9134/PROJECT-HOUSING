...
<div class="container my-4">
  ...
  <form method="post" action="<?= base_url('/admin/edit_property/' . ($property['id'] ?? '')) ?>" enctype="multipart/form-data" class="row g-3">
    <?= csrf_field() ?>
    ...
    <div class="col-md-6">
      ...
      <select id="seller_id" name="seller_id" class="form-select" required>
        <?php foreach ($sellers as $s): ?>
          <?php
            $currentSellerId = (string) ($property['seller_id'] ?? '');
            $selectedValue = (string) old('seller_id', $currentSellerId);
            $optionValue = (string) ($s['id'] ?? '');
          ?>
          <option value="<?= esc($optionValue) ?>" <?= ($optionValue === $selectedValue) ? 'selected' : '' ?>>
            <?= esc($s['name'] ?? '') ?> (ID: <?= esc($optionValue) ?>)
          </option>
        <?php endforeach; ?>
      </select>
    </div>
    ...
    <div class="col-md-12">
      ...
      <?php if (!empty($property['image_path'])): ?>
        <div class="mt-2">
          <img src="<?= esc($property['image_path']) ?>" alt="Property Image" style="height:150px;">
        </div>
      <?php endif; ?>
